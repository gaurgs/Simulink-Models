...
#include <stddef.h>
#include "blas.h"
#include "Energy_sfun.h"
#include "c2_Energy.h"
#include "mwmathutil.h"
#define CHARTINSTANCE_CHARTNUMBER      (chartInstance->chartNumber)
#define CHARTINSTANCE_INSTANCENUMBER   (chartInstance->instanceNumber)
#include "Energy_sfun_debug_macros.h"
...
static void initialize_c2_Energy(SFc2_EnergyInstanceStruct *chartInstance);
static void initialize_params_c2_Energy(SFc2_EnergyInstanceStruct *chartInstance);
static void enable_c2_Energy(SFc2_EnergyInstanceStruct *chartInstance);
static void disable_c2_Energy(SFc2_EnergyInstanceStruct *chartInstance);
static void c2_update_debugger_state_c2_Energy(SFc2_EnergyInstanceStruct
  *chartInstance);
static const mxArray *get_sim_state_c2_Energy(SFc2_EnergyInstanceStruct
  *chartInstance);
static void set_sim_state_c2_Energy(SFc2_EnergyInstanceStruct *chartInstance,
  const mxArray *c2_st);
static void finalize_c2_Energy(SFc2_EnergyInstanceStruct *chartInstance);
static void sf_c2_Energy(SFc2_EnergyInstanceStruct *chartInstance);
static void c2_chartstep_c2_Energy(SFc2_EnergyInstanceStruct *chartInstance);
static void initSimStructsc2_Energy(SFc2_EnergyInstanceStruct *chartInstance);
static void registerMessagesc2_Energy(SFc2_EnergyInstanceStruct *chartInstance);
static void init_script_number_translation(uint32_T c2_machineNumber, uint32_T
  c2_chartNumber);
static const mxArray *c2_sf_marshallOut(void *chartInstanceVoid, void *c2_inData);
static real_T c2_emlrt_marshallIn(SFc2_EnergyInstanceStruct *chartInstance,
  const mxArray *c2_N, const char_T *c2_identifier);
static real_T c2_b_emlrt_marshallIn(SFc2_EnergyInstanceStruct *chartInstance,
...
static void c2_eml_scalar_eg(SFc2_EnergyInstanceStruct *chartInstance);
static const mxArray *c2_c_sf_marshallOut(void *chartInstanceVoid, void
  *c2_inData);
static int32_T c2_c_emlrt_marshallIn(SFc2_EnergyInstanceStruct *chartInstance,
  const mxArray *c2_u, const emlrtMsgIdentifier *c2_parentId);
static void c2_b_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c2_mxArrayInData, const char_T *c2_varName, void *c2_outData);
static uint8_T c2_d_emlrt_marshallIn(SFc2_EnergyInstanceStruct *chartInstance,
  const mxArray *c2_b_is_active_c2_Energy, const char_T *c2_identifier);
static uint8_T c2_e_emlrt_marshallIn(SFc2_EnergyInstanceStruct *chartInstance,
  const mxArray *c2_u, const emlrtMsgIdentifier *c2_parentId);
static void init_dsm_address_info(SFc2_EnergyInstanceStruct *chartInstance);
...
static void initialize_c2_Energy(SFc2_EnergyInstanceStruct *chartInstance)
{
  ...
  chartInstance->c2_is_active_c2_Energy = 0U;
}
  ...
  *chartInstance)
{
  ...
  c2_d_hoistedGlobal = chartInstance->c2_is_active_c2_Energy;
  ...
  const mxArray *c2_st)
{
  ...
  *c2_y = c2_emlrt_marshallIn(chartInstance, sf_mex_dup(sf_mex_getcell(c2_u, 2)),
    "y");
  chartInstance->c2_is_active_c2_Energy = c2_d_emlrt_marshallIn(chartInstance,
    sf_mex_dup(sf_mex_getcell(c2_u, 3)), "is_active_c2_Energy");
  sf_mex_destroy(&c2_u);
  c2_update_debugger_state_c2_Energy(chartInstance);
...
static void sf_c2_Energy(SFc2_EnergyInstanceStruct *chartInstance)
{
  ...
  chartInstance->c2_sfEvent = CALL_EVENT;
  c2_chartstep_c2_Energy(chartInstance);
  _SFD_CHECK_FOR_STATE_INCONSISTENCY(_EnergyMachineNumber_,
...
static const mxArray *c2_sf_marshallOut(void *chartInstanceVoid, void *c2_inData)
{
  ...
  real_T c2_u;
  const mxArray *c2_y = NULL;
  SFc2_EnergyInstanceStruct *chartInstance;
  chartInstance = (SFc2_EnergyInstanceStruct *)chartInstanceVoid;
...
const mxArray *sf_c2_Energy_get_eml_resolved_functions_info(void)
{
  ...
  const mxArray *c2_b_is_active_c2_Energy, const char_T *c2_identifier)
{
  ...
  c2_y = c2_e_emlrt_marshallIn(chartInstance, sf_mex_dup
    (c2_b_is_active_c2_Energy), &c2_thisId);
  sf_mex_destroy(&c2_b_is_active_c2_Energy);
  return c2_y;
}
...
void sf_c2_Energy_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c2_Energy_get_autoinheritance_info(void)
{
...
mxArray *sf_c2_Energy_third_party_uses_info(void)
{
  mxArray * mxcell3p = mxCreateCellMatrix(1,0);
  return(mxcell3p);
}

static const mxArray *sf_get_sim_state_info_c2_Energy(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x4'type','srcId','name','auxInfo'{{M[1],M[9],T\"N\",},{M[1],M[8],T\"nvar\",},{M[1],M[5],T\"y\",},{M[8],M[0],T\"is_active_c2_Energy\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 4, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c2_Energy_get_check_sum(&mxChecksum);
  ...
  fullDebuggerInitialization)
{
  if (!sim_mode_is_rtw_gen(S)) {
    SFc2_EnergyInstanceStruct *chartInstance;
    chartInstance = (SFc2_EnergyInstanceStruct *) ((ChartInfoStruct *)
    ...
    if (ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
      /* do this only if simulation is starting */
      {
        ...
        if (chartAlreadyPresent==0) {
          ...
          init_script_number_translation(_EnergyMachineNumber_,
            chartInstance->chartNumber);
          sf_debug_set_chart_disable_implicit_casting
            (sfGlobalDebugInstanceStruct,_EnergyMachineNumber_,
      ...
      sf_debug_reset_current_state_configuration(sfGlobalDebugInstanceStruct,
        _EnergyMachineNumber_,chartInstance->chartNumber,
        chartInstance->instanceNumber);
    }
  }
}
...
static void sf_opaque_initialize_c2_Energy(void *chartInstanceVar)
{
  chart_debug_initialization(((SFc2_EnergyInstanceStruct*) chartInstanceVar)->S,
    0);
  initialize_params_c2_Energy((SFc2_EnergyInstanceStruct*) chartInstanceVar);
  initialize_c2_Energy((SFc2_EnergyInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_enable_c2_Energy(void *chartInstanceVar)
{
  enable_c2_Energy((SFc2_EnergyInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_disable_c2_Energy(void *chartInstanceVar)
{
  disable_c2_Energy((SFc2_EnergyInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_gateway_c2_Energy(void *chartInstanceVar)
{
  sf_c2_Energy((SFc2_EnergyInstanceStruct*) chartInstanceVar);
}

extern const mxArray* sf_internal_get_sim_state_c2_Energy(SimStruct* S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c2_Energy((SFc2_EnergyInstanceStruct*)
    chartInfo->chartInstance);         /* raw sim ctx */
  prhs[3] = (mxArray*) sf_get_sim_state_info_c2_Energy();/* state var info */
...
extern void sf_internal_set_sim_state_c2_Energy(SimStruct* S, const mxArray *st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  set_sim_state_c2_Energy((SFc2_EnergyInstanceStruct*)chartInfo->chartInstance,
    mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static const mxArray* sf_opaque_get_sim_state_c2_Energy(SimStruct* S)
{
  return sf_internal_get_sim_state_c2_Energy(S);
}

static void sf_opaque_set_sim_state_c2_Energy(SimStruct* S, const mxArray *st)
{
  sf_internal_set_sim_state_c2_Energy(S, st);
}

static void sf_opaque_terminate_c2_Energy(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc2_EnergyInstanceStruct*) chartInstanceVar)->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
      unload_Energy_optimization_info();
    }

    finalize_c2_Energy((SFc2_EnergyInstanceStruct*) chartInstanceVar);
...
{
  initSimStructsc2_Energy((SFc2_EnergyInstanceStruct*) chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c2_Energy(SimStruct *S)
{
  int i;
  ...
  if (sf_machine_global_initializer_called()) {
    initialize_params_c2_Energy((SFc2_EnergyInstanceStruct*)(((ChartInfoStruct *)
      ssGetUserData(S))->chartInstance));
  }
}

static void mdlSetWorkWidths_c2_Energy(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    mxArray *infoStruct = load_Energy_optimization_info();
...
static void mdlRTW_c2_Energy(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
...
static void mdlStart_c2_Energy(SimStruct *S)
{
  SFc2_EnergyInstanceStruct *chartInstance;
  chartInstance = (SFc2_EnergyInstanceStruct *)utMalloc(sizeof
    (SFc2_EnergyInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc2_EnergyInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway = sf_opaque_gateway_c2_Energy;
  chartInstance->chartInfo.initializeChart = sf_opaque_initialize_c2_Energy;
  chartInstance->chartInfo.terminateChart = sf_opaque_terminate_c2_Energy;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c2_Energy;
  chartInstance->chartInfo.disableChart = sf_opaque_disable_c2_Energy;
  chartInstance->chartInfo.getSimState = sf_opaque_get_sim_state_c2_Energy;
  chartInstance->chartInfo.setSimState = sf_opaque_set_sim_state_c2_Energy;
  chartInstance->chartInfo.getSimStateInfo = sf_get_sim_state_info_c2_Energy;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c2_Energy;
  chartInstance->chartInfo.mdlStart = mdlStart_c2_Energy;
  chartInstance->chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c2_Energy;
...
void c2_Energy_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c2_Energy(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c2_Energy(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c2_Energy(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c2_Energy_method_dispatcher.\n"
